...
void h5c__pack_tile(char* dst, const void* const* comps, size_t ncomp, size_t row0, size_t rows, size_t esize) {
    size_t c, r;
...
void h5c__unpack_tile(const char* src, void* const* comps, size_t ncomp, size_t row0, size_t rows, size_t esize) {
    size_t c, r;
 ...
 */
static h5c_status_t check_interleaved(h5c_file_t* file, const char* path, const void* const* comps, size_t ncomp, size_t n, h5c_type_t type, size_t* elemsize, size_t dims[2]) {
    h5c_status_t st;
    size_t i;

    if (ncomp == 0) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "ncomp is 0 for '%s'", (path != NULL) ? path : "(null)");
    }
    dims[0] = n;
    dims[1] = ncomp;
    ...
    if (comps == NULL) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "component pointer array is NULL for '%s'", path);
    }
    /*
    ...
    if (n > 0) {
        for (i = 0; i < ncomp; i++) {
            if (comps[i] == NULL) {
                return h5c__fail(H5C_ERR_INVALID_ARG, "component %lu is NULL for '%s'", (unsigned long)i, path);
            }
        }
    }
    *elemsize = h5c_type_size(type);
    if (*elemsize == 0) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "type %d has no interleaved mapping for '%s'", (int)type, path);
    }
...
static h5c_status_t open_for_write(h5c_file_t* file, const char* path, h5c_type_t type, const size_t dims[2], unsigned flags, hid_t* out_did) {
    h5c_status_t st;
    hid_t ftype, sid, did;
    int existed;
    ...
    *out_did = H5I_INVALID_HID;
    ...
    if (ftype == H5I_INVALID_HID) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "type %d has no numeric mapping for '%s'", (int)type, path);
    }
    ...
    if (existed) {
        ...
        if (sid < 0) {
            return h5c__fail_hdf5((long)sid, "cannot build dataspace for '%s'", path);
        }
        did = H5Dcreate2(file->fid, path, ftype, sid, h5c__lcpl(), H5P_DEFAULT, H5P_DEFAULT);
        H5Sclose(sid);
        if (did < 0) {
...
static h5c_status_t write_interleaved_impl(h5c_file_t* file, const char* path, const void* const* comps, size_t ncomp, size_t n, h5c_type_t type, unsigned flags) {
    h5c_status_t st;
    size_t dims[2], esize, row_bytes, rows, row0;
    hid_t mtype, did = H5I_INVALID_HID;
    char* stage;

    if ((st = check_interleaved(file, path, comps, ncomp, n, type, &esize, dims)) != H5C_OK) {
        return st;
    }
    if (file->readonly) {
    ...
    if (stage == NULL) {
        H5Dclose(did);
        return h5c__fail(H5C_ERR_NOMEM, "cannot allocate %lu staging bytes for '%s'", (unsigned long)(rows * row_bytes), path);
    }
...
h5c_status_t h5c_write_interleaved(h5c_file_t* file, const char* path, const void* const* comps, size_t ncomp, size_t n, h5c_type_t type, unsigned flags) {
    return h5c__record(file, write_interleaved_impl(file, path, comps, ncomp, n, type, flags));
}

static h5c_status_t read_interleaved_impl(h5c_file_t* file, const char* path, void* const* comps, size_t ncomp, size_t n, h5c_type_t type) {
    h5c_status_t st;
    h5c_dataset_info_t info;
    size_t dims[2], esize, row_bytes, rows, row0;
    hid_t mtype, did;
    ...
    if ((st = check_interleaved(file, path, (const void* const*)comps, ncomp, n, type, &esize, dims)) != H5C_OK) {
        return st;
    }
...
h5c_status_t h5c_read_interleaved(h5c_file_t* file, const char* path, void* const* comps, size_t ncomp, size_t n, h5c_type_t type) {
    return h5c__record(file, read_interleaved_impl(file, path, comps, ncomp, n, type));
}

static h5c_status_t read_component_impl(h5c_file_t* file, const char* path, void* buf, size_t comp, size_t n, h5c_type_t type) {
    h5c_status_t st;
    h5c_dataset_info_t info;
    ...
    if (info.rank != 2) {
        H5Dclose(did);
        return h5c__fail(H5C_ERR_SHAPE_MISMATCH, "'%s' has rank %d, expected an [n, ncomp] dataset", path, info.rank);
    }
    if (info.dims[0] != n) {
        H5Dclose(did);
        return h5c__fail(H5C_ERR_SHAPE_MISMATCH, "'%s' has %lu rows, expected %lu", path, (unsigned long)info.dims[0], (unsigned long)n);
    }
    if (comp >= info.dims[1]) {
        H5Dclose(did);
        return h5c__fail(H5C_ERR_INVALID_ARG, "component %lu is out of range for '%s' (ncomp %lu)", (unsigned long)comp, path, (unsigned long)info.dims[1]);
    }
    ...
    if (H5Sselect_hyperslab(fsid, H5S_SELECT_SET, start, NULL, count, NULL) < 0) {
        H5Sclose(fsid);
        H5Dclose(did);
        return h5c__fail_hdf5(-1, "cannot select component %lu of '%s'", (unsigned long)comp, path);
    }
    ...
    if (msid < 0) {
        H5Sclose(fsid);
        H5Dclose(did);
        return h5c__fail_hdf5((long)msid, "cannot build memory space for '%s'", path);
    }

    st = H5C_OK;
    if (H5Dread(did, mtype, msid, fsid, H5P_DEFAULT, buf) < 0) {
        st = h5c__fail_hdf5(-1, "H5Dread failed for component %lu of '%s'", (unsigned long)comp, path);
    }
    H5Sclose(msid);
    H5Sclose(fsid);
    H5Dclose(did);
...
h5c_status_t h5c_read_component(h5c_file_t* file, const char* path, void* buf, size_t comp, size_t n, h5c_type_t type) {
