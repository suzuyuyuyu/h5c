...
h5c_status_t h5c__check_common(h5c_file_t* file, const char* path, int rank, const size_t* dims) {
    int i;
    ...
    if (rank < 0 || rank > H5C_MAX_RANK) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "rank %d outside [0, %d] for '%s'", rank, H5C_MAX_RANK, path);
    }
    if (rank > 0 && dims == NULL) {
...
h5c_status_t h5c__info_from_dset(hid_t did, h5c_dataset_info_t* out) {
    ...
    if (ndims > H5C_MAX_RANK) {
        H5Sclose(sid);
        return h5c__fail(H5C_ERR_UNSUPPORTED, "dataset rank %d exceeds H5C_MAX_RANK", ndims);
    }
...
h5c_status_t h5c__shape_equals(const h5c_dataset_info_t* info, const char* path, int rank, const size_t* dims) {
    int i;

    if (info->rank != rank) {
        return h5c__fail(H5C_ERR_SHAPE_MISMATCH, "'%s' has rank %d, expected %d", path, info->rank, rank);
    }
    for (i = 0; i < rank; i++) {
        if (info->dims[i] != dims[i]) {
            return h5c__fail(H5C_ERR_SHAPE_MISMATCH, "'%s' dims[%d] is %lu, expected %lu", path, i, (unsigned long)info->dims[i], (unsigned long)dims[i]);
        }
    }
    return H5C_OK;
}
...
static h5c_status_t write_impl(h5c_file_t* file, const char* path, const void* buf, h5c_type_t type, int rank, const size_t* dims, unsigned flags) {
    h5c_status_t st;
    ...
    if (ftype == H5I_INVALID_HID || mtype == H5I_INVALID_HID) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "type %d has no numeric mapping (use the string API)", (int)type);
    }
...
h5c_status_t h5c_write(h5c_file_t* file, const char* path, const void* buf, h5c_type_t type, int rank, const size_t* dims, unsigned flags) {
    return h5c__record(file, write_impl(file, path, buf, type, rank, dims, flags));
}

static h5c_status_t read_impl(h5c_file_t* file, const char* path, void* buf, h5c_type_t type, int rank, const size_t* dims) {
    h5c_status_t st;
    h5c_dataset_info_t info;
    hid_t mtype, did;
...
h5c_status_t h5c_read(h5c_file_t* file, const char* path, void* buf, h5c_type_t type, int rank, const size_t* dims) {
    return h5c__record(file, read_impl(file, path, buf, type, rank, dims));
}

static h5c_status_t info_impl(h5c_file_t* file, const char* path, h5c_dataset_info_t* out) {
    h5c_status_t st;
    hid_t did;
...
h5c_status_t h5c_dataset_info(h5c_file_t* file, const char* path, h5c_dataset_info_t* out) {
    return h5c__record(file, info_impl(file, path, out));
}

static h5c_status_t read_alloc_impl(h5c_file_t* file, const char* path, h5c_type_t type, void** buf, h5c_dataset_info_t* info) {
    h5c_status_t st;
    ...
    *buf = NULL;
    ...
    if (esize == 0) {
        return h5c__fail(H5C_ERR_INVALID_ARG, "h5c_read_alloc does not support type %d", (int)type);
    }
    ...
    if (mem == NULL) {
        return h5c__fail(H5C_ERR_NOMEM, "cannot allocate %lu bytes for '%s'", (unsigned long)(local.count * esize), path);
    }

    st = read_impl(file, path, mem, type, local.rank, (local.rank > 0) ? local.dims : NULL);
    if (st != H5C_OK) {
        free(mem);
        return st;
    }
...
h5c_status_t h5c_read_alloc(h5c_file_t* file, const char* path, h5c_type_t type, void** buf, h5c_dataset_info_t* info) {
