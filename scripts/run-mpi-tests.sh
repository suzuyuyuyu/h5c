...
#========= Shell Script ==========
#
# Runs h5c's parallel tests. They must NEVER be launched on a login node, at
...
#
# The build must already be configured with H5C_ENABLE_PARALLEL=ON. Configure
# and compile on the login node (that part is cheap); this script only builds
# incrementally and runs.
#
...
mkdir -p stdout stderr log

# Current job links, for easy access while it runs.
...
if [[ ${#binaries[@]} -eq 0 ]]; then
    ...
    echo "Was the build configured with H5C_ENABLE_PARALLEL=ON?" >&2
    exit 1
fi

# Keep the HDF5 files each test writes out of the source tree.
workdir="${BUILD_DIR}/test"
cd "${workdir}"
...
set +x
echo "=== h5c MPI tests: ${#binaries[@]} binaries, ${failed} failed, ${NRANKS} ranks ==="
exit "${failed}"
